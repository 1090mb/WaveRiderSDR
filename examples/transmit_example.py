...
def main():
    ...
    # Display allowed frequency bands
    allowed_bands = transmitter.get_allowed_frequencies()
    logger.info("Allowed TX frequency bands:")
    for start, end in allowed_bands:
        logger.info(f"  {start/1e6:.2f} - {end/1e6:.2f} MHz")
    ...
    try:
        ...
        logger.info("Generating test tone at 10 kHz offset...")
        ...
        logger.info("Attempting to change frequency to 434.0 MHz...")
        if transmitter.set_frequency(434.0e6):
            logger.info("Frequency changed successfully")
        else:
            logger.error("Frequency change rejected (not in allowed bands)")
        
        logger.info("Transmission example complete!")
